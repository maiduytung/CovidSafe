...
public class HealthDeclarationViewModel extends ViewModel {

    private final MutableLiveData<String> query = new MutableLiveData<>();

    private final LiveData<Resource<List<HealthDeclaration>>> results;

    private final NextPageHandler nextPageHandler;

    @Inject
    HealthDeclarationViewModel(HealthDeclarationRepository healthDeclarationRepository) {
        nextPageHandler = new NextPageHandler(healthDeclarationRepository);
        results = Transformations.switchMap(query, query -> {
            if (query == null || query.trim().length() == 0) {
                return AbsentLiveData.create();
            } else {
                return healthDeclarationRepository.getAll(query);
            }
        });
    }

    @VisibleForTesting
    public LiveData<Resource<List<HealthDeclaration>>> getResults() {
        return results;
    }
    ...
    static class NextPageHandler implements Observer<Resource<Boolean>> {
        ...
        private final HealthDeclarationRepository repository;
        @VisibleForTesting
        boolean hasMore;

        @VisibleForTesting
        NextPageHandler(HealthDeclarationRepository repository) {
